...
pub fn run() {
    ...
    let scores = lines
        .iter()
        .map(|l| find_next_history_value(l))
        .collect::<Vec<i32>>();

    println!("The answer is: {:?}", scores.iter().sum::<i32>());
}

fn find_next_history_value(line: &str) -> i32 {
...
fn extrapolate(history: Vec<Vec<i32>>) -> i32 {
    ...
    for (index, _) in history.iter().enumerate() {
        ...
        let last_prev_item = last_history_line[last_history_line.len() - 1];

        last_extrapolated = last_extrapolated + last_prev_item;
    }
