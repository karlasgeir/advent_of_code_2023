...
fn parse_hand(line: &str) -> Hand {
    ...
    let mut hand_type = get_hand_type(&cards);
    hand_type = apply_jokers(hand_type, &cards);
...
fn apply_jokers(hand_type: HandType, cards: &Vec<Card>) -> HandType {
    let joker_count = cards.iter().filter(|c| c.value == 1).count() as i32;
...
fn card_label_to_card(card_label: &char) -> Card {
    let card_value = match card_label {
        ...
        'Q' => 12,
        'T' => 10,
        // Jokers have the lowest value in part 2
        'J' => 1,
